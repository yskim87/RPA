...
class AddRowDialog(QDialog):
    ...
    def get_row_data(self):
        ...
        return [self.class_edit.text().upper(), self.prefix_edit.text().upper(), self.itm_desc_edit.text().upper(),
                self.qty_edit.text().upper(), self.uom_edit.text().upper()]
...
class myWindow(QWidget):
    ...
    def clickTreeAddBtn(self):
        ...
        if not selected_item:
            self.show_alert('품번을 추가할 Class(Parent)를 선택하세요') #김영진_알람메시지
            return
